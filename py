    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                if 1 <= choice_num <= len(available_choices): chosen_option = available_choices[choice_num - 1]; break
                else: info_message = "無效的選項"
            except ValueError: info_message = "請輸入數字或 S/L"
            break
        ...
        for stat, value in effects.get("player", {}).items():
            player_stats[stat] = min(100, player_stats.get(stat, 0) + value) if stat == 'stamina' else player_stats.get(stat, 0) + value
            info_message += f"[{stat} {'+' if value > 0 else ''}{value}] "
        for npc, stats in effects.get("npcs", {}).items():
            for stat, value in stats.items():
                npc_stats[npc][stat] += value
                if stat == "affection": info_message += f"[{npc} 好感度 {'+' if value > 0 else ''}{value}] "
        ...
        if time_stats["period_index"] >= len(PERIODS): # 新的一天
            time_stats["period_index"] = 0
            time_stats["day"] += 1
            player_stats["stamina"] = min(100, player_stats.get("stamina", 0) + STAMINA_RECOVERY_ON_SLEEP)
            # 將睡眠恢復體力的訊息加到前面
            info_message = f"一夜好眠，體力恢復了 {STAMINA_RECOVERY_ON_SLEEP} 點！ " + info_message
            
            today = str(time_stats["day"])
            if today in scheduled_events:
                current_scene_id = scheduled_events[today]
            else:
                current_scene_id = "daily_router"
        else: # 同一天，下一個時段
            current_scene_id = chosen_option["next_scene"]

if __name__ == "__main__":
    try: main()
    except KeyboardInterrupt: print("\n\n遊戲已中斷。"); sys.exit(0)
    except Exception as e: print(f"\n\n發生未預期的錯誤: {e}"); import traceback; traceback.print_exc()
